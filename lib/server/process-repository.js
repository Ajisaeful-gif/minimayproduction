    ...
    headers.map(async (header) => {
      try {
        ...
        return {
          ...
          meja: header.meja ?? "",
          cekWarnaPola: header.cekWarnaPola ?? "",
          cekMarker: header.cekMarker ?? "",
          validasiData: header.validasiData ?? "",
          rows: rows.map((row) => ({
            sku: row.kodeSku,
            produk: row.namaProduk,
            model: row.model,
        ...
        console.error(`Gagal memuat row cutting untuk header ${header.id}.`, error);
        return null;
      }
    })
  );
...
export async function savePlanSewingRecord(record, userId) {
  ...
  return db.transaction(async (tx) => {
    ...
    const existingRows = await loadRows(planSewingRows, planSewingRows.planSewingId, headerId);
    const retainedRows = existingRows
      .filter((row) => {
        if (selectedColour) {
          return normalizeColourLabel(row.warna) !== selectedColour;
    ...
    if (retainedRows.length || normalizedIncomingRows.length) {
      await tx.insert(planSewingRows).values(
        [...retainedRows, ...normalizedIncomingRows.map((row) => ({
          ...
          warna: row.colour,
          qtyPlan: row.qtyPlan,
          qtyCutting: row.qtyCutting,
          qtyPlanSewing: row.qtyPlanSewing
        }))].map((row) => ({
